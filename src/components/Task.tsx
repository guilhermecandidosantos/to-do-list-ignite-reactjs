import { Check, CheckCircle, Circle, Divide, Trash } from "phosphor-react";
...
export function Task({task, onDelete, onCompleted}: Props) {
  return (
    <div className={styles.task}>
      {/* <button onClick={() => onCompleted(task.id)}>{task.isCompleted ? <CheckCircle className={styles.circle_check} size={20} /> : <Circle size={20}/>}</button> */}
      {/* className={task.isCompleted ? styles.checked : ""} */}
